...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetNode = function (nodeData) {
  ...
  if (self.inRows === true) {
    ...
    if (workingCell.name === 'c') {
      ...
      // ST_CellType
      console.log(workingCell, 'TYPE', workingVal, workingPart)
        ...
        default: // eslint-disable-line no-fallthrough
          if (self.options.formatting) {
            var numFmts = self.workBook.workBookStyles.filter(function (item) {
              return item.name === 'numFmt' || item.name === 'cellStyleXfs'
            })
            if (numFmts.length > 0) {
              var cellXfsIndex = self.workBook.workBookStyles.findIndex(function (item) {
              ...
              if (formatId) { // if formatId exists in workbookStyles
                var date = new Date((+workingVal - (25567 + 2)) * 86400 * 1000) // convert from excel format
