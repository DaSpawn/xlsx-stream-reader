...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetNode = function(nodeData){
	...
	if (self.inRows == true){
		...
		if(workingCell.name == 'c'){
          ...
          var formatId = xfs[workingCell.attributes.s].attributes.numFmtId;
          var format = numFmts.find(function (item) {
            return item.attributes.numFmtId === formatId
          });

          //format.attributes.formatCode === "General"
          self.workingRow.values[cellNum] = workingVal || "";

          if (format.attributes.formatCode === "DD/MM/YYYY") {
            var date = new Date((workingVal - (25567 + 2)) * 86400 * 1000);
            self.workingRow.values[cellNum] = date.getDate().toString().padStart(2, "0") + '/' + (date.getMonth() + 1).toString().padStart(2, "0") + '/' + date.getFullYear();
          }
